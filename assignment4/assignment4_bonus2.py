...
def prepareData_json(path):
    ...
    tweet_str = ''
    for tweet in tweets:
        tweet_str += tweet + eot_char

    return tweet_str, tweet_chars, K, char_to_ind, ind_to_char, eot_char
...
class RNN():
    ...
    def train(self, update_steps):
        ...
        for t in tqdm(range(update_steps)):
            ...
            if self.eot_char in sequence_labels:
                intermediary_h_t = np.zeros((self.m, self.seq_length))
            # set hprev
            ...
            grad_U, grad_V, grad_W, grad_b, grad_c = self.compute_gradients(intermediary_a_t, intermediary_h_t, hprev,
                                                                            intermediary_p_t, X, Y)
