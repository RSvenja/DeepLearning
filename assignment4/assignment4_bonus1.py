...
def prepareData_json(path):
    ...
    for tweet in data:
        ...
        tweet_chars = tweet_chars.union(set(text))
        tweets.append(text)

    all_chars = set(chr(i) for i in range(256))
    unused_chars = all_chars - tweet_chars

    if unused_chars:
        eot_char = unused_chars.pop()
        tweet_chars.add(eot_char)
    else:
        print("No unused char for end of tweet found")

    K = len(tweet_chars)
    char_to_ind = {val: id for id, val in enumerate(tweet_chars)}  # dict with char and index in set
    ind_to_char = {id: val for id, val in enumerate(tweet_chars)}

    return tweets, tweet_chars, K, char_to_ind, ind_to_char, eot_char
...
class RNN():
    def __init__(self, K, book_data, book_chars, char_to_ind, ind_to_char, eot_char, epsilon=1e-8, m=100, eta=0.1, seq_length=25):
        self.m=m
        self.K = K
        self.eta=eta
    ...
    def synthesize(self,  h0, x0, n, char_to_ind, ind_to_char):
        ...
        xNext = x0
        h_t = h0
        #seq_of_char_oh = seq_of_char_to_onehot(seq_of_char, self.K, char_to_ind)
        ...
        for t in range(n):
            ...
            label = np.random.choice(self.K, p=p_t[:, 0])
            #label = self.sample_a_label(p_t)
    ...
    def train(self, epochs):
        ...
        output= {new_list: [] for new_list in range(epochs)}
        sloss_list = []
        ...
        for epoch in tqdm(range(epochs)):
            ...
            if epoch == 0:
                X = seq_of_char_to_onehot(self.book_data[0], self.K, self.char_to_ind)
            intermediary_h_t = np.zeros((self.m, self.seq_length))
            hprev = intermediary_h_t[:, [-1]]
            output[epoch] = self.synthesize(hprev, X[:, [0]], 140, self.char_to_ind, self.ind_to_char)

            for id,tweet in tqdm(enumerate(self.book_data)):

                # set seq_length
                self.seq_length = len(tweet)
                # set hprev
                intermediary_h_t = np.zeros((self.m, self.seq_length)) # reset it for each tweet
                ...
                training_sequence = tweet
                sequence_labels = tweet[1:]+self.eot_char
                ...
                if t% 100 ==0 : # synthesize text
                    text = self.synthesize( hprev, X[:,[0]], 140, self.char_to_ind, self.ind_to_char)
                    print("".join(text))
    ...
    # main
tweet_path = 'Dataset/master_2018.json'
data, tweet_chars, K, char_to_ind, ind_to_char, eot_char = prepareData_json(tweet_path)
rnn = RNN(K, data, tweet_chars, char_to_ind, ind_to_char, eot_char)
